...
| v.3.0    |                        | 20260319 | 2.1.1-beta16 | 张学文 |            |
...
| **操作范围** | **功能** | **功能说明**                                                                                                      | **是否支持** |
| ------------ | -------- | ----------------------------------------------------------------------------------------------------------------- | ------------ |
| 应用管理     | 创建应用 | 是否支持基础业务场景设计，如创建应用、创建自定义组件、创建分组、发布应用、导入/导出场景、编辑应用、运行应用等操作 | 支持         |
...
访问[K-RPA Lite官网](https://www.kingsware.cn/krpalite/)，点击右上角【下载中心】按钮，页面跳转后，点击【下载】按钮 下载lite setup安装包；

![](images/image-1.png)



![alt text](<assets/K-RPA Lite User Manual/image.png>)

| **下载项**            | **下载项说明**                                                                                                                         |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| 下载 | 下载完整的应用程序安装包，包括客户端&流程自动化所需插件（如Chrome、Edge等浏览器插件）                                                  |
| 下载绿色版            | 仅下载K-RPA Lite应用程序，不包括流程自动化所需插件（如Chrome、Edge等浏览器插件）                                                       |
...
双击下载好的setup安装包 进入安装界面 逐步点击下一步 安装即可 会自动安装lite程序以及浏览器插件；

![alt text](<assets/K-RPA Lite User Manual/image-1.png>)
...
在设置页面 点击 【插件中心】可安装或更新需要的插件

![alt text](<assets/K-RPA Lite User Manual/image-2.png>)
...
![alt text](<assets/K-RPA Lite User Manual/image-3.png>)

### 2.2 界面功能模块概述

#### 2.2.1 界面入口说明

![alt text](<assets/K-RPA Lite User Manual/image-4.png>)

K-RPA Lite由6个部分构成：首页、任务、学习。

| **功能模块** | **功能描述**                                                                                                               |
| ------------ | -------------------------------------------------------------------------------------------------------------------------- |
| 首页         | 主要用于提供应用管理的主功能模块操作，用户可以通过功能快捷入口完成应用创建，自定义组件创建、应用发布、应用运行等操作       |
...
1. 用户场景：该页面主要提供快速创建应用、创建自定义组件、查看最近打开的应用等操作。

2. 操作步骤：最近打开项目-->单击【创建】-->选择【创建应用/创建自定义组件】-->进入IDE界面-->保存-->发布。
...
| **功能名称**   | **功能描述**                                                                     |
| -------------- | -------------------------------------------------------------------------------- |
| 最近打开项目   | 通过页签切换，管理最近打开的应用及自定义组件                                     |
| 创建应用       | 点击【创建】-【创建应用】即弹出新建应用窗口，支持快速创建应用                    |
| 创建自定义组件 | 点击【创建】-【创建自定义组件】即弹出新建自定义组件窗口，支持快速创建自定义组件  |
| 运行           | 点击【运行】运行当前应用并弹出运行日志，自定义组件不支持运行预览                 |
| 编辑           | 点击【编辑】跳转应用或自定义组件IDE界面                                          |
| 删除           | 点击【删除】删除一条应用或自定义组件信息。应用或自定义组件正在运行时，不允许删除 |
...
1. 用户场景：用户登录流程设计器时，支持创建一个自定义组件或自定义组件分组。完成流程设计后，单击【发布】将自定义组件进行正式发布。在应用设计过程中，可通过”我的自定义组件“进行重复调用。

2. 操作步骤：我的自定义组件-->单击【创建】-->选择【创建自定义组件】-->进入IDE界面-->保存-->发布。
...
| **功能名称**   | **功能描述**                                                         |
| -------------- | -------------------------------------------------------------------- |
| 我的自定义组件 | 通过页签切换，创建常用指令及指令管理                                 |
| 创建分组       | 点击【创建】-【创建分组】快速创建我的自定义组件分组                  |
| 创建自定义组件 | 点击【创建】-【创建自定义组件】快速我的自定义组件                    |
| 编辑           | 点击【编辑】跳转我的自定义组件IDE界面                                |
| 删除           | 点击【删除】删除一条自定义组件信息。自定义组件正在调用时，不允许删除 |
...
| 元素库       | 用于捕获、存放及展示网页元素，元素库中存放web元素 和win元素，可分别通过【网页自动化】和【软件自动化】自定义组件对对应的元素进行操作。目前支持标准模式捕获和深度模式捕获 |
...
| 自动保存策略 | 设置编辑应用或自定义组件自动保存的策略，支持30秒、1分钟、2分钟                          |
| 界面语言     | 支持中文和英文版切换                                                                    |
