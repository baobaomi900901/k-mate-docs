const difyChatbotConfig = {
    ...
    baseUrl: 'http://192.168.104.246:60973',
  };
...
export function createChatbot() {
  ...
  async function e() {
      if (p && p.token) {
          var e = new URLSearchParams(await async function() {
              ...
              return await Promise.all(Object.entries(e).map(async ([e,t]) => {
                  ...
                  e = new Response(new Blob([e]).stream().pipeThrough(new CompressionStream("gzip"))).arrayBuffer(),
          ...
          const i = `${p.baseUrl || `http://${p.isDev ? "dev." : ""}udify.app`}/chatbot/${p.token}?` + e;
          function o() {
              var e, t;
              window.innerWidth <= 640 || (e = document.getElementById(c),
              t = document.getElementById(a),
              e && t && ((t = t.getBoundingClientRect()).top - 5 > e.clientHeight ? (e.style.bottom = t.height + 5 + "px",
              e.style.top = "unset") : (e.style.bottom = "unset",
              e.style.top = t.height + 5 + "px"),
              t.right > e.clientWidth ? (e.style.right = "0",
              e.style.left = "unset") : (e.style.right = "unset",
              e.style.left = 0)))
          }
          function t() {
              const n = document.createElement("div");
              Object.entries(p.containerProps || {}).forEach( ([e,t]) => {
                  "className" === e ? n.classList.add(...t.split(" ")) : "style" === e ? "object" == typeof t ? Object.assign(n.style, t) : n.style.cssText = t : "function" == typeof t ? n.addEventListener(e.replace(/^on/, "").toLowerCase(), t) : n[e] = t
              }
              ),
              n.id = a;
    ...
    `);
              ...
              if (t.style.cssText = "display: flex; align-items: center; justify-content: center; width: 100%; height: 100%; z-index: 2147483647;",
              t.innerHTML = h.open,
              n.appendChild(t),
              ...
              n.addEventListener("click", function() {
                  ...
                  "none" === e.style.display ? document.removeEventListener("keydown", d) : document.addEventListener("keydown", d),
                  o()) : (n.appendChild(((e = document.createElement("iframe")).allow = "fullscreen;microphone",
    ...
    `,
              ...
              p.draggable) {
                  ...
                  document.addEventListener("mousemove", function(e) {
                      var t, n, o;
                      i && (s.style.transition = "none",
                      s.style.cursor = "grabbing",
                      (t = document.getElementById(c)) && (t.style.display = "none",
                      ...
                      "x" !== l && "both" !== l || s.style.setProperty(`--${a}-left`, Math.max(0, Math.min(t, n)) + "px"),
                      "y" !== l && "both" !== l || s.style.setProperty(`--${a}-bottom`, Math.max(0, Math.min(e, o)) + "px"))
                  }),
          ...
          2048 < i.length && console.error("The URL is too long, please reduce the number of inputs to prevent the bot from failing to load"),
  ...
  function d(e) {
      var t;
      "Escape" === e.key && (e = document.getElementById(c),
      t = document.getElementById(a),
      e) && "none" !== e.style.display && (e.style.display = "none",
      t.querySelector("div").innerHTML = h.open)
  }
  document.addEventListener("keydown", d),
